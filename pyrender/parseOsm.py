...
class parseOsm(handler.ContentHandler):
  ...
  def loadOsm(self, filename):
    ...
    try:
      ...
      print "Error loading %s. Exception: %s" % (filename, e)
      print dir(e)
  ...
  def startElement(self, name, attrs):
    ...
    if name in('node','way','relation'):
	  ...
	  'lon': float(self.nodes[int(attrs["ref"])][1]) / self.divisor}
      else:
        node = {
          'id': int(attrs.get('id')),
          'lat': float(attrs.get('y')) / self.divisor, 
          'lon': float(attrs.get('x')) / self.divisor}
	#'lat': float(self.nodes[int(attrs["ref"])][0]),
	#'lon': float(self.nodes[int(attrs["ref"])][1])}
      self.waynodes.append(node)
      #self.waynodes.append(self.nodes[int(attrs["ref"])])
    elif name == 'tag':
